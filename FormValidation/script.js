const formInputs = document.querySelectorAll('.form-control');
    ...
    function passwordValidate() {
        ...
        passConditions.forEach((condition, index) => {
            if (index === 0 && /[A-Z]/.test(password)) { // Uppercase check
                condition.classList.add('valid');
                condition.classList.remove('invalid');
            } else if (index === 1 && /[a-z]/.test(password)) { // Lowercase check
                condition.classList.add('valid');
                condition.classList.remove('invalid');
            } else if (index === 2 && /[0-9]/.test(password)) { // Number check
                condition.classList.add('valid');
                condition.classList.remove('invalid');
            } else if (index === 3 && password.length >= 8) { // Minimum length check
                condition.classList.add('valid');
                condition.classList.remove('invalid');
            } else {
